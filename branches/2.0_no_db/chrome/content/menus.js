...
function handleMenuCommand (self, item, location) {
    var mpdbrowser = document.getElementsByTagName("mpdbrowser")[0]
    var mpdplaylist = document.getElementsByTagName("mpdplaylist")[0]
    if (self.URL) {
        var s = self.URL
        for (x in item) {
...
function mpmMenu_contextShowing (event, location, activeItem) {
    ...
    for (var i=0;i<mpmMenu.items.length;i++) {
        if (mpmMenu.items[i] == "separator") {
            ...
            var e = createMenuNode(menu, mpmMenu.items[i], activeItem, location)
            readySep = true
        }
    }
